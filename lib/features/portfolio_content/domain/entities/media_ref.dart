...
abstract class MediaRef with _$MediaRef {
  ...
  static MediaRef videoEmbed(
    String url, {
    ImageRef poster = const ImageRef(),
  ...
  bool get isEmpty =>
      isVideo ? videoUrl.trim().isEmpty && image.isEmpty : image.isEmpty;

  bool get hasPlayableVideo => isVideo && videoUrl.trim().isNotEmpty;

  /// True for a GIF. Flutter animates one the whole time it is mounted and
  /// offers no way to pause it, so a renderer has to keep it out of the tree
  /// instead — see `AppMedia`.
  ///
  /// Sniffed from the path, or from the data URI an embedded ref carries. That
  /// is why the picker stores `data:<mime>;base64,…` rather than bare base64:
  /// it keeps the payload self-describing, and `AppImage` already strips the
  /// prefix before decoding.
  bool get isAnimatedImage {
    if (kind != MediaKind.image) return false;
    final value = image.value.trim().toLowerCase();
    return value.endsWith('.gif') || value.startsWith('data:image/gif');
  }
}
